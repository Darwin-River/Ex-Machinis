# /bin/bash
#
# Script that starts events engine
#
...
# Log file
LOG_FILE_PREFIX=${PLAT_HOME}/log/start_events_engine
...
then
  ...
  then
    ...
    log_msg "WARNING: Events engine with PID ${PID} seems stopped, restarting ..."
    rm -f ${PID_FILE}
  fi
fi
...
#-----------------------------------------------------------------
# Launch events engine and store its PID
#-----------------------------------------------------------------

nohup $PLAT_HOME/bin/events-engine &
ENGINE_PID=$!
save_pid

# Add the watchdog line to cron
(crontab -u ${USER} -l ; echo "*/1 * * * * sh -c '. $HOME/.profile; $PLAT_HOME/bin/scripts/start_events_engine.sh' >> /dev/null 2>&1") | crontab -u ${USER} -

log_msg "************************************************************"
log_msg "Running events engine with PID ${ENGINE_PID}"
log_msg "************************************************************"
